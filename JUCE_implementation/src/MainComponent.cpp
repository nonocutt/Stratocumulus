...
public:
	...
	void loadRNBOAudioProcessor()
	{
		...
		_audioProcessorEditor.reset(new CustomAudioEditor(_audioProcessor.get(),
														  _audioProcessor->getRnboObject()));
		if (_audioProcessorEditor) {
			if (auto* custom = dynamic_cast<CustomAudioEditor*>(_audioProcessorEditor.get()))

			addAndMakeVisible(_audioProcessorEditor.get());
			DBG("Mounted editor: " + juce::String::toHexString(
				(juce::pointer_sized_int) _audioProcessorEditor.get()));
			resized();
	...
	void openAudioSettings()
	{
		...
		// Dialog content uses the existing device manager
		auto* content = new juce::AudioDeviceSelectorComponent(
			...
			false     // hide advanced options (set true if you want them)
		);
